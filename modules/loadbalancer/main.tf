resource "azurerm_public_ip" "loadbalancer-publicip" {
  count               = 2
  name                = "${var.loadbalancer-publicip}-lbpip-3${format("%1d", count.index + 1)}"
  location            = var.location
  resource_group_name = var.resource_group
  domain_name_label   = "lbdns-3${format("%1d", count.index + 1)}"
...
resource "azurerm_lb" "loadbalancer" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.loadbalancer-publicip[0].id
  }

  frontend_ip_configuration {
    name                 = "PublicIPAddress_2"
    public_ip_address_id = azurerm_public_ip.loadbalancer-publicip[1].id
  }
...
resource "azurerm_network_interface_backend_address_pool_association" "loadbalancer_pool_association" {
  ...
  ip_configuration_name   = "lvm-2096-ipconfig-${each.value + 1}"
